...
- `toolkit/container.nu up --reload-egress` no longer strands a running agent. It used to delete and recreate the proxy first and only then notice the agent already existed, aborting with "a container named X already exists" — an error that reads as "nothing happened" while the agent had just lost its exit. It now reloads the policy in place and says so, or names the address change and tells you to recreate the agent.
