...
- `nu toolkit/container.nu reload-egress <name>` applies an edited allowlist to a container that is already up — it takes the agent's name and nothing else. It was `up --reload-egress`, which forced you to retype the workspace folders on a path that ignores them: an existing container's mounts cannot change.
- Applying an edited allowlist no longer strands a running agent. It used to delete and recreate the proxy for every edit — on Apple `container`, which has no static-IP flag, that is a chance for the exit address the agent was built with to move. A running proxy is now reconfigured in place (`squid -k parse`, then `-k reconfigure`, so a list that does not parse leaves the old one in force), and only a stopped proxy is recreated; when the address does change, it is named and you are told to recreate the agent.
