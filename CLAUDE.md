...
- Post-build verification: `cozy-module/verify.nu` — one check set, run by `cozy verify` **inside a sandbox only**: it errors out without `/etc/sandbox-persistent.sh`, so a host install has no verify path at all. Checks take a transport closure so the check logic is independent of how commands are carried. The module, autoload-file and env-var expectations derive from `vendored-repos.nuon`, the `docker-files/nushell-autoload/` glob and `bootstrap.nu`'s env exports — add one of those and it gets checked automatically. The tool list (`const tools`) and the three non-autoload files do not derive: they are spelled out by hand, so a new brew formula in `bootstrap.nu` needs a matching line here
