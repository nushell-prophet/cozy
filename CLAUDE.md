...
- Kit spec for `sbx run shell --kit ./kit/`: `kit/spec.yaml` — environment + commands.install mirror the Dockerfile ENV + RUN block; the kit clones cozy in-sandbox and runs the same `bootstrap.nu`
