...
- Kit spec for `sbx run shell --kit ./kit/`: `kit/spec.yaml` — environment + commands.install mirror the Dockerfile ENV + RUN block; `kit/files/` is generated by `toolkit/pack-kit.nu` (gitignored)
