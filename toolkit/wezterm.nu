...
export def "nu-complete wezterm background" []: nothing -> table<value: string, description: string> {
    [
...
export def attach-window [
    ...
    # the spawned job's id, so `job kill` can reach it; nothing with --no-job
