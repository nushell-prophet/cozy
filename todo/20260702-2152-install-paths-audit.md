...
!! but isn't it a default one liner from brew? if it is my intuition says to keep it.
...
> implement "detect container via `/.dockerenv` and create the env file cozy needs instead of requiring it"
...
> hm, I didn't notice any problems with it. I always config my own credentials via `git config` and I see claude as an author when it commit into git (though I didn't check jj for a long time). But anyway, it seems like now everything works (at least in `sbx` and `docker sandbox` correctly).
...
!! yes, that's pain. I don't want to push untested, but I have to because I can't test otherwise. Let's just document this for now
