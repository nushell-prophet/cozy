...
covers:                # source paths update-design reconciles this file against
  - docker-files/nushell-autoload/agent-nu-wrapper.nu
  - docker-files/agent-config.nu
...
The [`docker-files/`](../docker-files/) bits that land in the running sandbox. `bootstrap.nu` Step 3.5 copies `docker-files/nushell-autoload/*.nu` → `~/.config/nushell/autoload/` (loaded on every shell start) and `docker-files/agent-config.nu` one level above it, into `~/.config/nushell/` (loaded by nobody automatically — only by the wrappers that name it with `--config`), Step 6 appends `global-claude.md` to `~/.claude/CLAUDE.md`, and the `pbcopy` shim is installed to `~/.local/bin` (on every Linux, just before Step 1). The [`Dockerfile`](../Dockerfile)'s final `COPY` places `workspace-README.md` at `~/workspace/README.md`. `logo.ans` stays in place and is read by `cozy logo` and the build MOTD.
...
### agent-nu-wrapper.nu
Give a `nu` launched from inside a nushell session the agent config, so its output arrives as NUON instead of a box table. Fires only when `CLAUDECODE` is set, so the human's session keeps stock nu, and `--config` / `--no-config-file` / `--mcp` / `-n` opt out either way. It covers the path the bash wrapper in `/etc/sandbox-persistent.sh` cannot reach — chiefly the nushell MCP, where autoload files do load but `display_output` never fires, because nu-mcp does not link nu-cli, so no hook reaches the renderer. Measured on 3,819 session transcripts: of the 212 MCP results carrying box drawing, 87 were a nested `nu` — the largest single cause, well ahead of a pipeline ending in `| table` (16). The guard sits inside the body because `def` is scoped to its block, so `if … { def nu … }` would define nothing at all.
**Code:** [`docker-files/nushell-autoload/agent-nu-wrapper.nu`](../docker-files/nushell-autoload/agent-nu-wrapper.nu)
...
## agent-config.nu
The nushell config both wrappers name with `--config`, copied to `~/.config/nushell/agent-config.nu` by Step 3.5. Deliberately one level above the autoload dir: an autoload file loads into the human's interactive session too, and this one replaces the table renderer. It exists because `nu --commands` and `nu script.nu` load no config at all, so they render with stock defaults — an 80-column box table that wraps identifiers mid-word, drops columns into a bare `...` and hides nested values behind `[list 3 items]`. A human at a wide terminal never sees that; anything reading the output as text loses the data and cannot tell that it did. A `display_output` hook renders lists, records and ranges as NUON instead; binary still goes through `table`, because returning it untouched writes raw bytes where `table` gives a readable hex dump. Kept to the hook alone — adding the module set here measured 150ms per `nu` invocation against 2ms for this, and agents invoke nu constantly. The second wrapper, for `nu` typed in an agent Bash call, is the `nu()` function `bootstrap.nu` writes into `/etc/sandbox-persistent.sh`; `cozy verify` exercises both.
**Code:** [`docker-files/agent-config.nu`](../docker-files/agent-config.nu)
