...
[`../firewall/allowed-domains.txt`](../firewall/allowed-domains.txt) keeps unvetted code and data **out**. It is not containment for what leaves: `github.com` carries `git push`, so with any credential in the workspace it is a full outbound channel. A much smaller list would be needed if the goal were keeping data in.
