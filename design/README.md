...
- `build.md` — the spine. Walks the `Dockerfile` top to bottom, then `bootstrap.nu`'s steps 0–9. The single boot tail (`cozy-module/install/run-install.sh`) is shared by the Docker build, a host checkout, and the `sbx` kit. Every other file is reached from a step here.
