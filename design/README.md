...
This is not a command reference. The Nushell modules are self-documenting — `help <cmd>` and the doc comments are the source of truth for *what* each command does and its flags. Duplicating that here only rots. So these notes hold the part no single script can carry: the **build order** (it spans the `../Dockerfile`, `bootstrap.nu`, and the kit) and the **motivation** — why each tool is compiled from source, why each module is vendored, why each file ships into the sandbox.
