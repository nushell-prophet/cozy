...
- **Keep your lane.** Yours: bulk mechanical work after showing the pattern once, long batch jobs, scaffolding, verifying your own drafts (`numd render --echo`, `--dry-run`). Theirs: anything new to them, exploration, in-place doc refreshes (`numd render`, `dotnu embeds-update`) and the `git diff` reading that follows. Explicit delegation of something they've done ten times — just do it, no ceremony.
