...
A canvas session cannot be started from within a session: the Canvas style, the Stop hook, the canvas path in the new session's own instructions, and `$env.GI_CANVAS` for the hook all arrive with `claude` at launch.
...
`gi` on its own reports what is seeded in this repo and whether the asking session is bound to a canvas; a non-null `canvas` means this **is** a canvas session — say so and stop, there is nothing to launch.
