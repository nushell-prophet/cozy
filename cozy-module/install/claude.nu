# Installer module for Claude Code; `install` does the work.
export def main [] { }
...
export def install []: nothing -> nothing {
    # Why `--all`: bootstrap.nu does `use claude.nu`, and this module's `main`
    ...
    let external = which --all claude | where type == external
    ...
    # `--all` for the same reason as above — without it this assert fired on every
    # build, whatever the installer did.
    if (which --all claude | where type == external | is-empty) {
